...
30 files, ~15,307 lines, and **30+ files importing `@sculptcore/api`**. Almost
...
`scripts/` → `lite-mesh` edges, measured 2026-08-15:

| Site | Kind |
| --- | --- |
| `scripts/data_api/api_define.ts:63` | `import '../lite-mesh/litemesh.js'` — side-effect |
| `scripts/editors/view3d/view3d_draw_webgpu.ts:50` | `buildSolidTexturedWgsl` from `litemesh_wgsl.js` |
| `scripts/framework_api.ts:157` | `export {LiteMesh} from './lite-mesh/index.js'` — the hub re-exports it |
| `scripts/editors/view3d/tools/boxmodel.ts:27,28` | `SelectLoopLiteMeshOp`, `SelectNearestLiteMeshOp`, `localRay`, `LiteMesh` |
| `scripts/entry_point.js:70,75` | side-effect imports of `litemesh_test_scene.js` and `litemesh_default_scene.js` |
| `scripts/webgpu/stencil_compute.ts` | stranded — assigned here by the strategy |
| `scripts/renderengine/renderengine_realtime.ts:1253` | already duck-typed; verify it stays that way |

The `sculptcore*.ts` toolmode files (`sculptcore.ts:39`,
`sculptcore_bindings.ts:6`, `sculptcore_gpu_stroke.ts:21`,
`sculptcore_ops.ts:3`) also import LiteMesh — those **move with it**, so they
are not edges to cut.

Cuts, in order:

1. **`api_define.ts:63`** — replace the side-effect import with the P7/P8
...
2. **`framework_api.ts:157`** — delete. A host hub re-exporting a concrete
   geometry type is the coupling in its purest form. Consumers move to
   `@addon/litemesh/api`.
3. **`entry_point.js:70,75`** — default scene and test scene become
   ...
   entry point.
4. **`view3d_draw_webgpu.ts:50`** — `buildSolidTexturedWgsl` is shader
...
5. **`boxmodel.ts:27,28`** — the boxmodel toolmode moves into the addon
...
`check:layers` interpretable throughout.
