...
import {extrudeFaceRegion, extrudeFacesIndividual, meshSnapshotBytes, splitOffFaces} from './modeling.js'
...
> extends ToolOp<Inputs, Outputs> {
  ...
  _finish(data: LeafMeshData, out: RegionResult): void {
    for (const d of [Domain.VERT, Domain.EDGE, Domain.FACE] as SelectDomain[]) {
      selectAll(data.mesh, d, false)
    }

    applySelection(data.mesh, Domain.FACE, out.faces, 'add')
...
export const LEAFMESH_MODELING_OPS = [LeafMeshExtrudeRegionOp, LeafMeshExtrudeIndividualOp, LeafMeshSplitOffOp]
